...
class FFT{
	...
	public:
		/**
		 * @brief Default consructor that does the memory allocation and the plans setup
		 */
		FFT();
		/**
		 ...
		 * @param res : the result of the FFT representation
		 */
		...
		/**
		 * @brief Accessor to the input array for initialisation
		 * @return the pointer to the internal input array
		 */
		inline double *get_in() const{return in;}
};
...
namespace fft{
	...
	void tensor_product(fftw_complex *res_fft, const fftw_complex *lhs_fft, const fftw_complex *rhs);
}
